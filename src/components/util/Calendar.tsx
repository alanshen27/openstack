...
export default function Calendar({ onChange }: { onChange: (date: { day: number, month: Month | number, year: number, week?: Date[] }) => void, 
    active?: { day: number, month: Month | number, year: number } }) {
                    ...
                    <div key={index} className="flex flex-col items-center">
                        ...
                        <div className="flex flex-col space-y-3 mt-3">
                            {weekstoDays[index].map((day, dayIndex) => (
                                <div key={dayIndex} className={`w-6 h-6 hover:cursor-pointer flex justify-center items-center p-2 rounded-full hover:bg-gray-100 ${day.prev ? 'text-gray-400' : 'text-black'} ${(active.day === day.label && !day.prev && date.month === active.month && date.year === active.year) && 'bg-black text-white'}`} 
                                    onClick={() => {
                                        if (day.prev) {
